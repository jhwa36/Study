...
//ResourceBundleMessageSource의 setBaseName()을 통해 전달 받은 파일을, setMessageSource()를 자동으로 호출해서 매핑한다.
//MessageSourceAware와 ResourceBundleMessageSource는 쌍으로 사용한다.

public class MemberInfo implements MessageSourceAware {
	
	private MessageSource message;	//전달된 properties
									//properties는 변수 = 값 형태로 되어 있고 읽어서 파싱한다.
